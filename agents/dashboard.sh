#!/bin/bash
# dashboard.sh — Dashboard agenti: status corrente + task in coda
# Uso: ./agents/dashboard.sh
#
# Chi coordina chiama questo script per sapere chi sta lavorando, su cosa,
# e quanti task ha ancora in coda ogni agente.
#
# CODA = task esplicitamente accodati con qtask.sh add (non messaggi inbox)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/lib/team.sh"

team_require_manifest

SHARED_DIR="${OFFICE_SHARED_DIR:-$SCRIPT_DIR/../shared-context}"
STATUS_FILE="$SHARED_DIR/AGENT-STATUS.json"
QUEUES_DIR="$SHARED_DIR/queues"

python3 - "$(team_manifest_path)" "$STATUS_FILE" "$QUEUES_DIR" <<'PYEOF'
import json, os, sys
from datetime import datetime

MANIFEST, STATUS_FILE, QUEUES_DIR = sys.argv[1], sys.argv[2], sys.argv[3]

# Il manifest è la verità: un agente presente solo in AGENT-STATUS.json
# (perché rimosso dal team) non viene mostrato.
team = json.load(open(MANIFEST))["team"]

try:
    with open(STATUS_FILE) as f:
        sdata = json.load(f)
except (FileNotFoundError, json.JSONDecodeError):
    sdata = {}


def queue_count(agent):
    path = os.path.join(QUEUES_DIR, f"{agent}.json")
    if not os.path.isfile(path):
        return 0
    try:
        with open(path) as f:
            return len(json.load(f))
    except (json.JSONDecodeError, OSError):
        return 0


STATUS_ICON = {
    "WORKING": "● WORKING",
    "IDLE":    "○ IDLE   ",
    "STANDBY": "◌ STANDBY",
}

now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
W = 74

print()
print("  " + "━" * W)
print(f"  THE OFFICE — DASHBOARD{now:>{W - 22}}")
print("  " + "━" * W)
print(f"  {'AGENTE':<16}  {'RUOLO':<14}  {'STATUS':<9}  {'CODA':>5}  TASK CORRENTE")
print(f"  {'─'*16}  {'─'*14}  {'─'*9}  {'─'*5}  {'─'*24}")

totals = {"WORKING": 0, "IDLE": 0, "STANDBY": 0}
total_q = 0

for m in team:
    agent = m["id"]
    s = sdata.get(agent, {"status": "STANDBY", "task": "", "ts": ""})
    status = s.get("status", "STANDBY")
    task = s.get("task", "") or "—"
    q = queue_count(agent)
    icon = STATUS_ICON.get(status, "◌ STANDBY")
    totals[status] = totals.get(status, 0) + 1
    total_q += q
    q_str = f"[{q}]" if q > 0 else "[ ]"
    role = m["role"]
    if len(role) > 14:
        role = role[:13] + "…"
    if len(task) > 24:
        task = task[:21] + "..."
    print(f"  {m['name']:<16}  {role:<14}  {icon}  {q_str:>5}  {task}")

print("  " + "━" * W)
print(f"  WORKING: {totals['WORKING']}   "
      f"IDLE: {totals['IDLE']}   "
      f"STANDBY: {totals['STANDBY']}   "
      f"TASK IN CODA: {total_q}")
print("  " + "━" * W)
print()
PYEOF
